...
try:
    ...
    class OptizelleProblem(Optizelle.ScalarValuedFunction):
...
class OptizelleSolver(OptimizationSolver):
    ...
    def __build_optizelle_state(self):
        ...
        # FIXME: support constraints
        assert self.problem.constraints is None

        # Set up the optizelle scalar functions 
        ...
        self.fns.f = OptizelleProblem(self.problem.reduced_functional, scale=scale)
    ...
    def solve(self):
        ...
        # FIXME: Return a list of length 1 of the user provided the control as a list
        if len(self.state.x) == 1:
