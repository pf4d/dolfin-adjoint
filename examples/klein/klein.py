...
# However, this approach quickly becomes
# infeasible if the number of input variables grows or 
...
# input variables.
# This allows us to compute sensitivities with respect
# to hundreds of input variables, or even with respect to entire input functions!
#
# In the following example we consider a time-dependent model and apply dolfin-adjoint to 
# determine the sensitivity of the final solution with respect to changes in its initial condition. 
#
# Problem definition
# ******************
#
...
# 
# The aim of this example is to compute the sensitivity of :math:`J` on the initial condition :math:`g`, that is:
#
# .. math::
...
# The reason is that for this example the domain :math:`\Omega` is a closed 
...
# <http://paulbourke.net/geometry/klein/>`_ with parameters a = 2.0, n = 2 and m = 1. The meshed Klein bottle looks like this:
...
# be done here.
# The code for generating this mesh, can be found  
# in ``examples/klein/make_mesh.py`` in the ``dolfin-adjoint``
# source tree.

# Next we create the required functions to solve the heat equation. 
...
# Finally, we define the initial temperature and the thermal diffusivity coefficient.
...
# By multiplying the heat equation with a testfunction :math:`v \in V`, integrating the Laplace term by parts, and 
# applying a backward Euler time-discretisation, the discrete problem reads: Given :math:`u_{\textrm{old}} \in V`, find :math:`u \in V` such that for all :math:`v \in V`:
...
# One remark before we continue with solving the forward problem.
# Generally, the adjoint equations depend on the solutions of the forward model.
# Therefore, dolfin-adjoint stores every forward solution in memory by default. 
# While this approach is fast, it requires significant memory which can be quickly become
# infeasible for large-scale, time-dependent applications. For such situations, 
# an optimal checkpointing strategy based on the revolve library :cite:`griewank2000` may be used, which trades off memory 
# requirement with additional computational cost. The following code demonstrates how checkpointing 
# would be activated:

#adj_checkpointing('multistage', steps=11, snaps_on_disk=1, snaps_in_ram=3, verbose=True)

# We leave checkpointing deactivated for now, but will present runtime results with checkpointing at the end of this section.
# More information on checkpointing can found in the :doc:`checkpointing section <../../documentation/checkpointing>`.
...
# the initial condition :math:`g` is copied into :math:`u_{\textrm{old}}`. Note the annotate=True
# argument, which tells dolfin-adjoint that this assignment is part of the forward model computation. 
# Without it, the model output would have no dependency on the initial condition :math:`g` and the sensitivity 
# would just be 0.
# Also note the :py:func:`adj_inc_timestep <dolfin_adjoint.adj_inc_timestep>` call. 
# This function indicates the end of a time step and is only required with checkpointing enabled. 

# At this point, we can define the objective functional :math:`J` and compute the sensitivity with respect
# to the initial condition :math:`g`:
...
# Note the project=True flag for :py:func:`compute_gradient`. It indicates that the 
# gradient should not be returned as an operator, that is not in the dual space :math:`V^*`, but instead
# its Riesz representation in the primal space :math:`V`. This is necessary to plot the sensitivities without seeing 
# mesh dependent features. 
...
# The following image on the left shows the initial temperature variation :math:`u(T=0)` and the image on the right the final temperature variation :math:`u(T=1)`.
# The diffusion of the initial temperature variation over the time period is clearly visible.

# .. image:: u_combined.png
#     :scale: 30
#     :align: center

# The next image shows the computed sensitivity :math:`\textrm{d} (\|u(t=1)\|) / \textrm{d}(u(T=0))`:
...
# In the following test, we investigate the additional computational cost when using checkpointing over the default store-all strategy in dolfin-adjoint.
# The following table compares the slow-down factor with 11 timesteps, no disk checkpoints, varying memory checkpoints
