...
DOLFIN $1.2$ is not supported by the \verb+timestepping+ library.
...
\section{Explicit linear multi-step}

Explicit linear multi-step (or Adams-Bashforth) schemes take the form:
\begin{equation}
  T^{n + 1} = T^n + \Delta t \sum_{i = 1}^N \gamma_i F( T^{n + 1 - i}),
\end{equation}
where the $\gamma_i$ are constants. Explicit linear multi-step schemes therefore
require the values of $F(T)$ from $N$ previous time levels. Given a choice of
$N$, the $\gamma_i$ are chosen so as to yield $N$th order accuracy. If $N > 1$
then the scheme is not self starting, and care is required when initialising in
order to maintain the accuracy of the method. Further details of explicit linear
multi-step schemes can be found in \citet[section 2.1]{iserles2009} and
...
method used to perform the first timestep, and compute $T^1$, must be a least
...
and $T^1$, must be a least second order accurate. Hence, for example, second
...
\section{Implicit linear multi-step}

Implicit linear multi-step (or Adams-Moulton) schemes take the form:
\begin{equation}\label{eqn:adams_moulton}
  T^{n + 1} = T^n + \Delta t \sum_{i = 0}^{N - 1} \gamma_i F( T^{n + 1 - i}),
\end{equation}
where the $\gamma_i$ are constants. Implicit linear multi-step schemes therefore
require the values of $F(T)$ from $N - 1$ previous time levels. Given a choice of
$N$, the $\gamma_i$ are chosen so as to yield $N$th order accuracy. If $N > 2$
then the scheme is not self starting, and care is required when initialising in
order to maintain the accuracy of the method. Further details of implicit linear
multi-step schemes can be found in \citet[section III.1]{hairer1987}.

In an implicit linear multi-step scheme the right-hand-side of equation
...
For $N = 1$ the single coefficient $\gamma_0$ is chosen to be:
\begin{equation}
  \gamma_0 = 1,
\end{equation}
yielding:
...
The first order explicit Runge-Kutta scheme is:
\begin{subequations}
  \begin{equation}
